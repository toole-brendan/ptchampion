...
TAG="ptchampionacr.azurecr.io/ptchampion-api:fix-db-schema-${TIMESTAMP}"

echo "===== Deploying Database Schema Fix ====="
...
docker build --platform linux/amd64 -t ${TAG} -f Dockerfile.fix .
...
echo ""
echo "Deployment completed! The database schema fix has been applied."
