...
StyleDictionary.registerFormat({
  ...
  formatter: function({ dictionary, platform }) {
    ...
    colorTokens.filter(token => token.path.includes('base')).forEach(token => {
      ...
      output += `    public static let ${swiftName} = Color("${colorName.charAt(0).toUpperCase() + colorName.slice(1).replace(/-([a-z])/g, (_, g) => g.toUpperCase())}", bundle: .module)\n`;
    });
    ...
    dictionary.allTokens
      ...
      .forEach(t => {
        const swiftName = t.path[t.path.length - 1]                 // heading1
          .replace(/-([a-z])/g, (_, g) => g.toUpperCase());          // kebab → camel
        output += `    public static let ${swiftName}: CGFloat = ${t.value}\n`;
      });

    // blank line between size constants and family helpers
    output += `\n`;
    ...
    dictionary.allTokens
      ...
      .forEach(t => {
        const swiftName = t.path[t.path.length - 1]                 // heading, bodyBold…
          .replace(/-([a-z])/g, (_, g) => g.toUpperCase());
        output += `    public static func ${swiftName}(size: CGFloat) -> Font {\n`;
        output += `      Font.custom("${t.value}", size: size, relativeTo: .body)\n`;
        output += `    }\n\n`;
      });
    ...
    dictionary.allTokens
      ...
      .forEach(t => {
        const swiftName = t.path.slice(-1)[0]
          .replace(/-([a-z])/g, (_, g) => g.toUpperCase());
        // ensure uniqueness
        output += `    public static let radius${swiftName.charAt(0).toUpperCase()+swiftName.slice(1)}: CGFloat = ${t.value}\n`;
      });
    output += `  }\n`;
    ...
    /* ---------- SHADOWS ---------- */
    /* Shadows skipped until a proper RGBA → Color converter is written */
