...
struct SitupView: View {
    ...
    @StateObject private var viewModel = SitupViewModel()
        ...
        ZStack {
            if viewModel.isLoading {
                ...
                VStack(spacing: 20) {
                    ...
                    Text("Unable to load sit-up exercise data.")
...
class SitupViewModel: ObservableObject {
    ...
    func loadExercise() {
        ...
        APIClient.shared.getExercises()
            ...
            .sink(receiveCompletion: { [weak self] completion in
                ...
                if let situpExercise = exercises.first(where: { $0.type == .situp }) {
                    self?.exercise = situpExercise
                }
                self?.isLoading = false
            })
