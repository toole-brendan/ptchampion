...
-- No PostGIS required - just for global rankings
...
BEGIN
    ...
    LOOP
        -- Insert user if not exists (without PostGIS fields)
        INSERT INTO users (
    ...
    -- Generate workout data for each user (last 7 days only)
    FOR user_record IN 
    ...
    LOOP
        -- Generate workouts for the last 7 days
        FOR i IN 0..6 LOOP
            ...
            VALUES (
                ...
                true,
                workout_date + INTERVAL '6 hours' + INTERVAL '1 minute' * (i * 7),
                NOW()
            )
            ...
            VALUES (
                ...
                true,
                workout_date + INTERVAL '7 hours' + INTERVAL '1 minute' * (i * 7),
                NOW()
            )
            ON CONFLICT DO NOTHING;
            ...
            IF i % 2 = 0 THEN
                ...
                IF final_score > 0 THEN
                    ...
                    VALUES (
                        ...
                        true,
                        workout_date + INTERVAL '8 hours' + INTERVAL '1 minute' * (i * 7),
                        NOW()
                    )
                    ON CONFLICT DO NOTHING;
                END IF;
            ...
            IF i % 3 = 0 THEN
                ...
                VALUES (
                    ...
                    true,
                    workout_date + INTERVAL '15 hours' + INTERVAL '1 minute' * (i * 7),
                    NOW()
                )
                ON CONFLICT DO NOTHING;
            END IF;
...
-- Check overall leaderboard (aggregate)
SELECT 
    u.first_name || ' ' || u.last_name as name,
    u.location,
    COUNT(DISTINCT w.exercise_type) as exercises_completed,
    ROUND(AVG(w.grade)::numeric, 1) as avg_grade
FROM users u
JOIN workouts w ON u.id = w.user_id
WHERE w.is_public = true
  AND w.completed_at >= CURRENT_DATE - INTERVAL '7 days'
GROUP BY u.id, u.first_name, u.last_name, u.location
HAVING COUNT(DISTINCT w.exercise_type) = 4  -- Must have all 4 exercises
