...
import { detectPose, detectPushup, PushupState } from "@/lib/tensorflow";
import { calculatePushupGrade, getScoreRating } from "@/lib/exercise-grading";
...
export default function PushupPage() {
  ...
  const [exerciseState, setExerciseState] = useState<PushupState>({
    isUp: false,
    isDown: false,
    count: 0,
    formScore: 0,
  ...
  const exercise = (exercises as any[] || []).find((e: any) => e.type === "pushup");
  ...
  useEffect(() => {
    ...
    const detectPoseLoop = async () => {
      ...
      try {
        ...
        if (pose) {
          const newState = detectPushup(pose, exerciseState);
  ...
  const completeMutation = useMutation({
    mutationFn: async () => {
      ...
      // Calculate grade
      const grade = calculatePushupGrade(exerciseState.count);
