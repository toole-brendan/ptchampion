...
import { detectPose, detectPullup, PullupState } from "@/lib/tensorflow";
import { calculatePullupGrade, getScoreRating } from "@/lib/exercise-grading";
...
export default function PullupPage() {
  ...
  const [exerciseState, setExerciseState] = useState<PullupState>({
    isUp: false,
    isDown: false,
    count: 0,
    formScore: 0,
  ...
  const exercise = (exercises as any[] || []).find((e: any) => e.type === "pullup");
  ...
  useEffect(() => {
    ...
    const detectPoseLoop = async () => {
      ...
      try {
        ...
        if (pose) {
          const newState = detectPullup(pose, exerciseState);
  ...
  const completeMutation = useMutation({
    mutationFn: async () => {
      ...
      // Calculate grade
      const grade = calculatePullupGrade(exerciseState.count);
    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200">
        <div className="container px-4 py-3 mx-auto flex items-center justify-between">
          ...
          <h2 className="text-2xl font-bold">Pull-ups</h2>
          <div></div>
        </div>
      </header>
                  ...
                  <div>
                    ...
                    <div className="text-sm text-slate-500">
                      {formAnalysis.position === 'good' ? 'Good body alignment' : 
                       formAnalysis.position === 'warning' ? 'Body swinging slightly' : 
                       'Poor body control'}
                    </div>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="text-sm text-slate-500">
                      {formAnalysis.depth === 'good' ? 'Full range of motion' : 
                       formAnalysis.depth === 'warning' ? 'Moderate range of motion' : 
