...
class DashboardViewModel: ObservableObject {
    ...
    func refresh() {
        ...
        // Calculate next future date based on last workout date
        var futureDate = Date()
        if let lastWorkoutDate = allWorkouts.first?.endTime, 
           let next = Calendar.current.date(byAdding: .day, value: 1, to: lastWorkoutDate) {
            futureDate = next
        }
